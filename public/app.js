...
function renderOwner(){
  ...
  <div class="filters">
    <button class="chip ${state.filterTrade==='all'?'active':''}" data-trade="all">Tous les corps de métier</button>
    ${Object.entries(TRADES).map(([k,t])=>`<button class="chip ${state.filterTrade===k?'active':''}" data-trade="${k}"><span class="dot" style="background:${t.color}"></span>${t.label}</button>`).join('')}
  </div>
...
function attachOwnerEvents(){
  document.querySelectorAll('.filters .chip').forEach(c=>{ c.onclick=()=>{ state.filterTrade=c.dataset.trade; render(); }; });
...
/* ===================== MODALS ===================== */
function closeModal(){ document.getElementById('modalRoot').innerHTML=''; stopRecordingIfAny(); state.capture={photos:[],audio:null}; }
...
function openProofModal(id){
  ...
  state.capture={photos:[],audio:null};
  document.getElementById('modalRoot').innerHTML=`
  <div class="overlay" id="ov">
    <div class="modal">
  ...
  document.getElementById('submitProof').onclick=async()=>{
    if(state.capture.photos.length===0 && !state.capture.audio){ toast('Ajoutez au moins une photo ou une note vocale'); return; }
    try{
      await api(`/tasks/${t.id}/proof`,{method:'PATCH',body:JSON.stringify({photos:state.capture.photos,audio:state.capture.audio})});
