...
router.post('/login', (req, res) => {
  ...
  const user = db.users.findByEmail(String(email).toLowerCase().trim());
...
router.get('/users', authenticate, requireRole('owner'), (req, res) => {
  const users = [...db.users.all()].sort((a, b) => (a.role === b.role ? 0 : a.role === 'owner' ? -1 : 1));
  res.json({ users: users.map(publicUser) });
});

router.post('/users', authenticate, requireRole('owner'), (req, res) => {
  const { email, password, name, role, trade } = req.body || {};
  if (!email || !password || !name || !role) {
    return res.status(400).json({ error: 'Champs manquants.' });
  }
  if (!['owner', 'artisan'].includes(role)) {
    return res.status(400).json({ error: 'Rôle invalide.' });
  }
  if (role === 'artisan' && !TRADES.includes(trade)) {
    return res.status(400).json({ error: 'Corps de métier invalide pour un artisan.' });
  }
  ...
  const user = db.users.insert({
    ...
    name,
    role,
    trade: role === 'artisan' ? trade : null,
  });
  res.status(201).json({ user: publicUser(user) });
});

router.delete('/users/:id', authenticate, requireRole('owner'), (req, res) => {
